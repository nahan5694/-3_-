...
if (btnApCollect) {
    toggleMenuRedDot(btnApCollect, PlayerData.pendingLiveTickets >= 30);
}
      let maxAP = 120;
...
// 2-1. 사무소 버튼 (scene-11): 제작 지시한 작업이 모두 완료된 경우
const btnOffice = document.querySelector('.menu-btn[data-target="scene-11"]');
let hasOfficeComplete = false;

if (isFeatureUnlocked('scene-11') && PlayerData.office) {
  for (const type of Object.keys(PlayerData.office)) {
    if (type === 'supplies' || type === 'lounge') continue;

    const state = PlayerData.office[type];

    if (state && state.craftCompleteNotice) {
      hasOfficeComplete = true;
      break;
    }
  }
}

toggleMenuRedDot(btnOffice, hasOfficeComplete);
...
checkFeatureUnlocksAndAnnounce();
  ...
  // AP System Modal
  const btnApCollectModal = document.getElementById("btn-ap-collect");
  ...
  if(btnApCollectModal) {
    btnApCollectModal.addEventListener("click", () => {
  ...
  if(btnApClaim) {
    ...
    const btnApCollect = document.getElementById("btn-ap-collect");
    if (btnApCollect) {
        toggleMenuRedDot(btnApCollect, false);
    }
